...
class YBTextPicker: UIViewController {
    ...
    func show(withAnimation animationType:YBTextPickerAnimation){
        ...
        if let topController = UIApplication.topViewControllersample() {
    ...
    override func viewDidLoad() {
        ...
        if UIView.appearance().semanticContentAttribute == .forceRightToLeft {
               if let tfOfSearchBar = self.txtSearch.value(forKey: "searchField") as? UITextField { tfOfSearchBar.textAlignment = .right }
               }
               else {
                   if let tfOfSearchBar = self.txtSearch.value(forKey: "searchField") as? UITextField { tfOfSearchBar.textAlignment = .left }
               }
...
//            if let tColor = appearance.titleTextColor{
                titleLabel.textColor = HELPER.hexStringToUIColor(hex: SESSION.getAppColor())
//            }
...
//            if let dBtnColor = appearance.doneButtonColor{
                btnDone.setTitleColor(HELPER.hexStringToUIColor(hex: SESSION.getAppColor()), for: .normal)
//            }
        ...
        case .phone:
            paddingToTop.constant = 0
            paddingToBottom.constant = 0
...
extension UIApplication {
    class func topViewControllersample(controller: UIViewController? = UIApplication.shared.keyWindow?.rootViewController) -> UIViewController? {
        if let navigationController = controller as? UINavigationController {
            return topViewControllersample(controller: navigationController.visibleViewController)
        }
        if let tabController = controller as? UITabBarController {
            if let selected = tabController.selectedViewController {
                return topViewControllersample(controller: selected)
            }
        }
        if let presented = controller?.presentedViewController {
            return topViewControllersample(controller: presented)
        }
        return controller
    }
}
